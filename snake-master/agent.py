...
class Agent:
    def get_move(self, board, score, turns_alive, turns_to_starve, direction):
        ...
        if (board[current[0]+1][current[1]] == GameObject.EMPTY and (current[0]+1,current[1]) not in closed):
            frontier.append((current[0]+1,current[1]))
        if(board[current[0]-1][current[1]] == GameObject.EMPTY and (current[0]-1,current[1]) not in closed):
            frontier.append((current[0]-1,current[1]))
        if (board[current[0]][current[1]+1] == GameObject.EMPTY and (current[0],current[1]+1) not in closed):
        ...
        for x in range(len(frontier)):
            print(x)
            #if ((abs(food[0] - frontier[x][0]) + abs((food[0][1] - frontier[x][1])):
